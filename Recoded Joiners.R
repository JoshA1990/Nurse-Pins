...
library(tidyverse)
source("C:/Users/Josh.Andrews/OneDrive - Department of Health and Social Care/Documents/R Codes/Nurse-Pins/PIN Variables.R")
...
Data_esr_grouped <- Data_birth_filter |>
  ...
  mutate(
    NMC_to_ESR = case_when(
      ...
      .data$registration_date >= months_7 ~ "5 to 6 Months",
...
test <- Data_esr_grouped |> 
  mutate(NQN = ifelse(joiner == 1 & NMC_to_ESR %in% joiner_nqn & country_of_training %in% domestic_trained |
                        occ_joiner ==1 & NMC_to_ESR %in% occ_joiner_nqn & country_of_training %in% domestic_trained, 1, 0)) |>
  mutate(retire_and_return = ifelse(joiner == 1 & NMC_to_ESR %in% c("Over 10 years", "5 to 10 years", "2 to 5 years") & Age_In_Years_y2 > 55 |
                                    occ_joiner == 1 & NMC_to_ESR %in% c("Over 10 years", "5 to 10 years", "2 to 5 years") & Age_In_Years_y2 > 55  , 1, 0)) |>
  mutate(IR = ifelse(joiner == 1 & NMC_to_ESR %in% occ_joiner_nqn & country_of_training %in% c("Overseas (non EU)", "EU") |
                    occ_joiner == 1 & NMC_to_ESR == "Immediate Joiner" & country_of_training %in% c("Overseas (non EU)", "EU"), 1, 0))

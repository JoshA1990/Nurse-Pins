# Summary of non-nationality and training country matches
#Snapshot of each month rather than a comparison of joiners and leavers
...
Raw_Data <- read_csv("Staff in Post - Workforce 202312 with PINS.csv")
...
Nurses <- Raw_Data %>% 
  ...
  mutate(Org_region = case_when(Ocs_Code %in% orgs_region_east_england ~ "East England",
                                ...
                                Ocs_Code %in% orgs_region_north_west ~ "North West England",
                                Ocs_Code %in% orgs_region_south_west ~ "South West England",
                                Ocs_Code %in% orgs_region_southeast ~ "South East England"))
...
non_matches_summary <- training_non_matches |>
  group_by(Nationality_grouping, country_of_training, Org_region) |>
...
write.csv(summary, paste0("C:/Users/Josh.Andrews/OneDrive - Department of Health and Social Care/Nurse Data/Outputs/NMC Pins/Training nationality mismatch/",substr(Data$Tm_Year_Month,1,8)[1],".csv"))
